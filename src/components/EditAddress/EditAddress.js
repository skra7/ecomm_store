...
const useStyles = makeStyles((theme) => ({
 ...
 function EditAddress(props) {
  ...
  const [varState, setVarState] = React.useState({
    ...
    addressLine1: props.location.state.addressLine1 ||"",
    addressLine2: props.location.state.addressLine2 || "",
  ...
  const handleAddress = (event) => {
   ...
   addressStatus=true;
    var updatedaddress = {
      id :props.location.state.addressId,
...
{ 
  ...
  props.onSetStatus();
  history.push({pathname : `${props.location.state.locate}`,
...
const mapStateToProps = (state) => {
  return {
    ...
    error:state.address.updateerror,
    status:state.address.updatestatus
  };
};
const mapDispatchToProps=(dispatch)=>{
  return{
     onUpdateaddress:(updatedaddress)=>dispatch(AddressAction.UpdateAddress(updatedaddress)),
